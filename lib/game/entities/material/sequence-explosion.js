ig.module(
    'game.entities.material.sequence-explosion'
)
.requires(
    'impact.entity'
)
.defines(function(){
    ...
    EntitySequenceExplosion = ig.Entity.extend({
        //粒子數
        particles: 10,
        ...
        //目前產生幾個, 因為 init 時, 會先產生一個, 所以預設為 1
        spawn: 1,
        ...
        update: function() {
            if ( this.spawn < this.particles && this.intervalTimer.delta() >= 0) {
                if ( this.radius.max  == this.radius.min ) {
                    var radius = this.radius.max;
                }
                else {
                ...
                var settings = this.particleSettings;
                settings.radius = radius;
                ig.game.spawnEntity(EntityDeathExplosionParticle, this.pos.x, this.pos.y, settings);                    
